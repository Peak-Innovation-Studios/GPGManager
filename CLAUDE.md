...
- **Release build numbers are monotonic across both channels via the appcast.** `sparkle:version` (CFBundleVersion) must only increase or Sparkle won't offer the update. Xcode Cloud uses its own incrementing `CI_BUILD_NUMBER` (currently ~126); GitHub Actions has no equivalent counter, so `github-release.sh` derives the build number as `max(sparkle:version in the published appcast) + 1` when one isn't passed explicitly (do **not** use `github.run_number` — it's unrelated and far lower). Commit count is not viable (the repo has ~50 commits, below the build sequence).
- **GitHub auto-creates a published, *immutable* release when a tag is pushed**, and immutable releases reject asset uploads. Both release scripts handle this: delete any existing published release, (re)create it as a **draft**, upload the DMG while it's mutable, then PATCH `draft:false` to publish.
