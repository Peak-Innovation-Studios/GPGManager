...
**The v1.0.3 release raced itself.** The tag push fired *both* release channels at once — Xcode Cloud shipped flawlessly (build 128: appcast, GitHub Release, Homebrew tap all landed within a minute), while the GitHub Actions Release job died at 0.2 seconds with exit 1 and *zero* output. The autopsy: the script's first act is deriving the build number by curling the appcast, and under `set -euo pipefail` a failing command substitution kills the script **before the `else` fallback written for exactly that case can run** — the fallback was unreachable dead code, and curl's stderr went to `/dev/null`, hence the silent corpse. The likely trigger is poetic: the Actions curl hit the appcast at the very moment Xcode Cloud's post-build script was rewriting the same R2 object. Lesson: under `set -e`, `VAR=$(pipeline)` *is* a failure point — if you wrote a fallback for it, you must append `|| true` inside the substitution or the fallback is fiction. Corollary lesson: a 0.2-second failure with no output means "died before the first echo" — read the script top-down from the entry point, not from the middle.

**Fixes that fell out:** `release.yml` is now **manual-only** (`workflow_dispatch`) so tag pushes trigger only Xcode Cloud and the channels can never race; the appcast fetch got retries and a reachable failure path that **dies loudly instead of falling back to `GITHUB_RUN_NUMBER`** (a regressed build number silently bricks Sparkle updates — a failed job is strictly better).

**The 1.0.4 release proved the fallback channel — because Xcode Cloud went dark.** Two pushes to `main` and three tag events produced zero Xcode Cloud builds. The check-run timeline exposed a long-held misconception: the workflow was *never* tag-triggered — 1.0.3's "tag release" was actually the **merge-to-main branch trigger** firing at 04:53:51, three minutes *before* the tag existed; the release only worked because the tag showed up before the publish step ran. Diagnosis by elimination: GitHub healthy (no incidents, xcode-cloud app installed/unsuspended/unchanged since May), Apple status green, yet Xcode Cloud's manual-start picker couldn't even *see* the v1.0.4 tag → its per-product repo mirror was stuck. Remedy is reconnecting the GitHub grant in App Store Connect. Meanwhile the manual-only Actions workflow shipped 1.0.4 (build 129) end-to-end — the exact scenario it was rebuilt for, one day earlier.

**The appcast fetch failures were never a race.** The same derivation failed again on a quiet appcast — while succeeding from a dev machine every time. Two-for-two failures from GitHub runners vs. consistent local success = Cloudflare bot protection challenging Azure datacenter IPs. Fix: `github-release.sh` now reads the appcast over the **R2 S3 API** (the same object the release writes, with credentials already in the env) and only falls back to the public URL — WAF-immune. Later confirmed with receipts via the Cloudflare GraphQL firewall log: `managed_challenge` from Bot Fight Mode against **ASN 8075 (Microsoft/Azure)** on `/gpg-manager/appcast.xml` at 04:57:19Z — the exact second the v1.0.3 run died — and again for the v1.0.4 attempt. The same log showed `link_maze_injected` events against residential ASNs on the appcast path, i.e. **real Sparkle clients occasionally get maze-altered responses** — bot defenses on a machine-readable update feed hurt exactly the clients the feed exists for. Lesson: when CI fails at something that "works on my machine", suspect the *vantage point* (IP reputation, egress rules) before suspecting timing.

**Then 1.0.4 turned out to be unlaunchable — and was pulled.** The Actions-built DMG passed codesign verification, notarization, stapling, and Gatekeeper — and died instantly at spawn ("Launchd job spawn failed", POSIX 163). Root cause: `keychain-access-groups` is a *restricted* entitlement, and a Developer ID app must carry an **embedded provisioning profile** authorizing it. Xcode Cloud's Direct Distribution export embeds one automatically; `github-release.sh`'s hand-rolled `resign_app` signs with the entitlements plist and no profile → AMFI kills the process before `main()`. Nothing in the pipeline ever *launched* the built app, so every gate passed and the corpse shipped to the appcast, GitHub Release, and Homebrew — the Actions channel has likely never produced a runnable binary. Poetic detail: the broken app's pinentry helper is AMFI-dead too, so the incident briefly broke *commit signing on the dev machine* ("No pinentry") — the release manager's own tooling was collateral. Mitigation: pruned appcast entry 1.0.4/129 (+ R2 enclosure), deleted the GitHub Release, reverted the Homebrew cask and website card to 1.0.3. Fixes needed before the fallback channel is trusted again: embed Developer ID provisioning profiles for **both** bundle IDs in `resign_app`, and add a **spawn test** to CI (launch the built app, fail the job if it dies) — signature-valid is not launchable. Also learned: `brew reinstall` uses the local tap clone; after a remote cask revert, `brew update` first or you reinstall the same bad bytes from cache.

**Release notes joined the pipeline.** Until now nothing user-facing carried notes — appcast items had no description, and the GitHub Release body was "Automated release from Xcode Cloud build N." Borrowed Agent Toolkit's convention: the **annotated tag body is the single source of release notes** (`- ` bullets). Both release scripts now put the bullets into the GitHub Release body, and after the DMG asset is confirmed they dispatch a new `update-changelog.yml` workflow in `peak-innovation-studios-web` that runs the site's existing `update-changelog.mjs` (which already anticipated `--app gpg-manager`) and pushes the regenerated changelog card — Cloudflare Pages deploys it. The 1.0.3 notes were backfilled by hand: re-annotate the tag *locally only* (the pushed tag is locked by the immutable release), run the site script, restore the local tag from origin, `gh release edit` the body. Needs one new secret before the next release: `WEBSITE_DISPATCH_TOKEN` (fine-grained PAT, Actions write on the site repo) in both Xcode Cloud env and the GPGManager Actions secrets — the existing PATs are scoped to other repos and can't dispatch there.
